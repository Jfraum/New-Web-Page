    ...
    <div className="z-0 mt-10 md:px-2 md:mx-auto md:masonry">
      {images.map((image, index) => (
        <motion.div
          ...
          initial="hidden"
          whileInView="visible"
          // Configuración para que la animación se repita siempre al hacer scroll
          viewport={{ once: false, amount: 0.2, margin: "0px 0px -50px 0px" }}
          className="z-0 relative mb-6 break-inside-avoid"
        >
          <div className="group relative overflow-hidden rounded-md bg-zinc-900 shadow-lg">
            <Link
              className="absolute inset-0 z-10 flex items-center justify-center text-[#ffe8d6] text-2xl font-bold bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity duration-300 cursor-pointer text-center px-4"
              to={image.href}
            >
              {image.text}
            </Link>
